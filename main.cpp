...
#ssb
    ...
    orderByResult = rangeData.OrderBy[value];
    groupByResult = rangeData.GroupBy[value % 2];
    selectResult = rangeData.Where[value < 3].Select[value * 6];
    distinctResult = rangeData.Concat(MyRange<int>({5, 5, 6})).Distinct().Reverse();
    allGreaterThanZero = rangeData.All[value > 0];
    anyGreaterThanThree = rangeData.Any[value > 3];
    containsThree = rangeData.Contains(3);
    elementAtTwo = rangeData.ElementAt(2);
    toSetResult = rangeData.ToSet();
    toListResult = rangeData.Where[value > 4].ToList();
    toDequeResult = rangeData.ToDeque();
    toVectorResult = rangeData.ToVector();
    minResult = rangeData.Where[value > 2].Select[value * 2].Min();
    maxResult = rangeData.Where[value > 2].Select[value * 2].Max();
    averageResult = rangeData.Where[value > 2].Select[value * 2].Average();
    countResult = rangeData.Where[value > 2].Select[value * 2].Count();
#esb
